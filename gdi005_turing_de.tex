...
Dann wird noch eine zweite Turingmaschine (gleiche Kodierung) $\mathbf{D}$ definiert, die sich wie folgt verhält (siehe Abbildung \ref{halteproblem_fig2}):
\begin{itemize}
...
\paragraph{Beispiel Collatz-Problem}
...
%Weil es keine Turingmaschine $\mathbf{H}$ gibt, die das Halte-Problem löst, kann die Informatik diese Frage, ob das Programm bei jeder beliebigen %Eingabe $n$ endet oder nicht, auch nicht beantworten.