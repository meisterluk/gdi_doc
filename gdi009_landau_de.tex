...
\begin{enumerate}
  ...
  \item Eine Funktion der Struktur $ab^{cn + d}$ (mit $a$, $b$, $c$ und $d$ als Konstanten) reduzieren wir auf $e^n$ mit $e=b^c$. Schließlich ist $ab^{cn + d} = a b^d b^{cn}$ wobei $ab^d$ konstant ist. Weiters gilt die Rechenregel $n^{km} = {(n^k)}^m$ womit sich $e^n$ ergibt.
\end{enumerate}
